          ...
          <div key={n} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold
                ${done ? 'bg-rust text-white' : active ? 'bg-charcoal text-white' : 'bg-gray-200 text-gray-400'}`}>
                {done ? '✓' : n}
              </div>
              <span className={`text-xs mt-1 ${active ? 'text-charcoal font-semibold' : 'text-gray-400'}`}>{label}</span>
            </div>
            {i < steps.length - 1 && (
              <div className={`h-0.5 flex-1 mx-1 mb-5 ${done ? 'bg-rust' : 'bg-gray-200'}`} />
            )}
          </div>
        )
      })}
...
function Step1({ onSelect }: { onSelect: (s: Service) => void }) {
  return (
    <div>
      <h2 className="text-xl font-bold text-charcoal mb-1">Hvad ønsker du at booke?</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {services.map((s) => (
          <button
            key={s.id}
            onClick={() => onSelect(s)}
            className="text-left bg-white border-2 border-warm-gray hover:border-charcoal rounded-lg p-6 transition-colors group"
          >
            <span className="text-3xl">{s.icon}</span>
            <h3 className="text-lg font-bold text-charcoal mt-3 mb-2 group-hover:text-charcoal-mid">{s.name}</h3>
            <p className="text-sm text-gray-500 mb-3">{s.description}</p>
            <p className="text-rust font-semibold text-sm">Fra {s.priceFrom.toLocaleString('da-DK')} kr.</p>
          </button>
        ))}
      </div>
    </div>
...
function Step2({
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-charcoal mb-1">Båd og ejer</h2>
        ...
        {[
          ...
          <div key={key}>
            ...
            <input
              ...
              className="w-full border border-warm-gray rounded px-3 py-2 text-sm focus:outline-none focus:border-charcoal"
            />
          </div>
        ))}
      ...
      <div className="mb-8">
        <label className="block text-sm font-medium text-charcoal mb-3">Vælg størrelse</label>
            ...
            <button
              ...
              className={`border-2 rounded-lg p-4 text-left transition-colors
                ${selectedSize?.id === size.id
                  ...
                  : 'border-warm-gray bg-white hover:border-charcoal'}`}
            >
              ...
              <p className={`text-sm font-bold mt-2 ${selectedSize?.id === size.id ? 'text-rust-light' : 'text-rust'}`}>
      ...
      <div className="flex gap-3">
        <button onClick={onBack} className="px-5 py-2 border border-charcoal text-charcoal rounded hover:bg-charcoal hover:text-white transition-colors text-sm">
          Tilbage
        </button>
        <button
          onClick={onNext}
          disabled={!canProceed}
          className="px-6 py-2 bg-charcoal text-white rounded text-sm font-medium hover:bg-charcoal-mid transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Fortsæt
        </button>
      </div>
...
function Step3({
  ...
  if (service.type !== 'kranløft') {
    return (
      <div>
        <h2 className="text-xl font-bold text-charcoal mb-1">Periode</h2>
        <p className="text-gray-500 text-sm mb-6">Du booker: <strong>{service.name} · {selectedSize.label}</strong></p>
        <div className="bg-white border border-warm-gray rounded-lg p-6 mb-8 max-w-md">
          <p className="text-sm text-gray-500 mb-1">Sæson</p>
          <p className="font-bold text-charcoal text-lg">Oktober 2026 – April 2027</p>
          ...
          <div className="mt-4 pt-4 border-t border-warm-gray">
            ...
            <p className="text-2xl font-bold text-rust">{selectedSize.price.toLocaleString('da-DK')} kr.</p>
            <p className="text-xs text-gray-400">{selectedSize.priceUnit}</p>
          </div>
        </div>
        <div className="flex gap-3">
          <button onClick={onBack} className="px-5 py-2 border border-charcoal text-charcoal rounded hover:bg-charcoal hover:text-white transition-colors text-sm">Tilbage</button>
          <button onClick={onNext} className="px-6 py-2 bg-charcoal text-white rounded text-sm font-medium hover:bg-charcoal-mid transition-colors">Fortsæt</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-charcoal mb-1">Vælg tidspunkt</h2>
      ...
      {Object.entries(slotsByDate).map(([date, slots]) => (
        <div key={date} className="mb-6">
          <h3 className="text-sm font-semibold text-charcoal mb-3 uppercase tracking-wide">{formatDate(date)}</h3>
                ...
                <button
                  ...
                  className={`border-2 rounded-lg p-3 text-center transition-colors
                    ${full ? 'border-gray-100 bg-gray-50 text-gray-300 cursor-not-allowed'
                      : selected ? 'border-charcoal bg-charcoal text-white'
                      : 'border-warm-gray bg-white hover:border-charcoal'}`}
                >
...
function Step4({
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-charcoal mb-1">Bekræft booking</h2>
      <p className="text-gray-500 text-sm mb-6">Gennemse detaljerne og klik Bekræft.</p>

      <div className="bg-white border border-warm-gray rounded-lg overflow-hidden mb-6 max-w-lg">
        {rows.map(({ label, value }) => (
          <div key={label} className="flex px-5 py-3 border-b border-warm-gray last:border-0">
            <span className="text-sm text-gray-400 w-28 shrink-0">{label}</span>
            <span className="text-sm font-medium text-charcoal">{value}</span>
          </div>
        ))}
        <div className="flex px-5 py-4 bg-offwhite">
          <span className="text-sm font-bold text-charcoal w-28 shrink-0">Total</span>
          <span className="text-lg font-bold text-rust">{selectedSize.price.toLocaleString('da-DK')} kr.{selectedSize.priceUnit}</span>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        <button onClick={onBack} className="px-5 py-2 border border-charcoal text-charcoal rounded hover:bg-charcoal hover:text-white transition-colors text-sm">Tilbage</button>
        <button onClick={onConfirm} className="px-8 py-2 bg-rust hover:bg-rust-light text-white rounded text-sm font-bold transition-colors">
          Bekræft booking
        </button>
      </div>
    </div>
...
function ConfirmationView({
  ...
  return (
    <div className="text-center max-w-md mx-auto py-8">
      ...
      <h2 className="text-2xl font-bold text-charcoal mb-2">Booking bekræftet!</h2>
      <p className="text-gray-500 mb-6">Vi har registreret din booking og sender en kvittering til <strong>{form.email}</strong>.</p>

      <div className="bg-white border border-warm-gray rounded-lg p-6 text-left mb-6">
        <p className="text-xs text-gray-400 mb-1">Bookingnummer</p>
        <p className="font-bold text-charcoal text-lg mb-4">{bookingId}</p>
        ...
        <p className="text-sm mt-3 font-bold text-rust">{selectedSize.price.toLocaleString('da-DK')} kr.{selectedSize.priceUnit}</p>
      </div>

      <Link href="/" className="inline-block bg-charcoal hover:bg-charcoal-mid text-white font-medium px-8 py-3 rounded transition-colors">
